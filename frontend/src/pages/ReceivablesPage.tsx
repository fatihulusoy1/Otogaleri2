...
function getCollectedAmount(item: ReceivablePayable): number {
...
export function ReceivablesPage() {
  ...
  async function loadItems() {
    ...
    setLoading(true);
    try {
      const response = await api.getReceivablePayables(session.token, 1);
      setItems(response);
      setError("");
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : "Alacak listesi yüklenemedi.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSettle() {
    ...
    try {
      ...
      setError(settleError instanceof Error ? settleError.message : "Tahsilat işlemi kaydedilemedi.");
    } finally {
  ...
  async function handlePartialSettle() {
    ...
    if (!amount) {
      setError("Kısmi tahsilat tutarı girmeniz gerekiyor.");
      return;
    }

    if (amount > partialTarget.remainingAmount) {
      setError("Girilen tutar kalan alacaktan büyük olamaz.");
      return;
    }
    ...
    try {
      ...
      setError(settleError instanceof Error ? settleError.message : "Kısmi tahsilat kaydedilemedi.");
    } finally {
      setProcessingId(null);
    }
  }
  ...
  return (
    <section className="page-stack">
      <PageHeader
        eyebrow="Alacaklar"
        title="Vadeli satış, çek ve senet takibi"
        description="Müşteri alacaklarını, vadesi gelen evrakları ve kapanan tahsilatları araçlar ekranındaki düzende izleyin."
      />
      ...
      <article className="panel">
        <div className="debt-sticky-stack">
          ...
          <div className="filter-bar debt-filter-bar">
            ...
            <label>
              <span>Kişi adı</span>
              <input value={nameFilter} placeholder="Müşteri adına göre ara" onChange={(event) => setNameFilter(event.target.value)} />
            </label>
        ...
        <div className="vehicles-table">
          <div className="vehicles-table-body">
                ...
                <article key={item.id} className={`vehicles-row debt-row debt-card ${isExpanded ? "expanded" : ""}`}>
                  ...
                  <div className="debt-card-body">
                    ...
                    <div className="vehicles-cell vehicles-actions debt-actions">
                      ...
                      >
                        {processingId === item.id ? "İşleniyor..." : "Tahsil edildi"}
                      </button>
                      <button
                        type="button"
            ...
            {!loading && filteredItems.length === 0 ? (
              <div className="empty-panel">Bu filtrede alacak kaydı bulunmuyor.</div>
            ) : null}
          </div>
        </div>
      </article>
      ...
      {partialTarget ? (
        <div className="modal-backdrop">
          <div className="panel modal-panel data-form">
            <div className="modal-header">
              <div className="panel-heading">
                <h2>Kısmi tahsilat girin</h2>
                <p>{partialTarget.counterpartyName} / Kalan {formatCurrency(partialTarget.remainingAmount)} / Tahsil edilen {formatCurrency(getCollectedAmount(partialTarget))}</p>
              </div>
